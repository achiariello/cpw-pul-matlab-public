...
% Frequency range: 100 kHz .. 1 GHz (logspace).
% Line length: 0.8 mm.
% Assumptions:
%   - Cpul and Lpul constant with frequency
%   - Gpul = 0
%   - Rpul(f) from skin-depth / penetration model
...
% Export Touchstone
outfile = fullfile(this_dir, 'gaas_gold_cps_diff_2port_100kHz_1GHz_0p8mm.s2p');
...
xlabel('Frequency [Hz]');
ylabel('Magnitude [dB]');
title('Differential CPS S-parameters (100 kHz .. 1 GHz, 0.8 mm, Z0 = 100 Ohm)');
legend('|S11|', '|S21|', 'Location', 'best');

plotfile = fullfile(this_dir, 'gaas_gold_cps_diff_2port_100kHz_1GHz_0p8mm_plot.png');
exportgraphics(fig, plotfile, 'Resolution', 180);
close(fig);

fprintf('Touchstone file written:\n%s\n', out.filename);
fprintf('Plot written:\n%s\n', plotfile);
fprintf('lineLength: %.3f mm\n', lineLength * 1e3);
fprintf('f-range: %.3f kHz .. %.3f GHz, N=%d\n', fmin/1e3, fmax/1e9, Nfreq);
